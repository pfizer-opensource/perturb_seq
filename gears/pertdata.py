...
class PertData:
    ...
    def load(self, data_name = None, data_path = None):
        ...
        if os.path.isfile(dataset_fname):
            ...
            self.dataset_processed = pickle.load(open(dataset_fname, "rb")) ##dictionary with key: pert, e.g. 'CREB1+ctrl', value: list of Data elements with x, y, pert_idx, de_idx, pert 
            print_sys("Done!")
        else:
    ...
    def get_dataloader(self, batch_size, test_batch_size = None):
        ...
        # Create cell graphs
        cell_graphs = {} ##seems to be a dictionary with keys in ['train','val','test'], value: list of torch_geometric.data.data.Data objects with x, y, pert_idx, de_idx, pert
        if self.split == 'no_split':
            ...
            print("PertData construction: ", type(cell_graphs["train"]), type(cell_graphs["train"][0]), cell_graphs["train"][0:10])
