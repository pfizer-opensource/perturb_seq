...
def rank_genes_groups_by_cov(
    ...
    print(f"cov_categories: {cov_categories}")
    for cov_cat in cov_categories:
        #name of the control group in the groupby obs column
        control_group_cov = '_'.join([cov_cat, control_group])
        print(f"control_group_cov: {control_group_cov}")
        ...
        de_genes = pd.DataFrame(adata_cov.uns['rank_genes_groups']['names']) ##columns will be headed by perturbation group, e.g. "K562(?)_DAD1+ctrl_1+1" and value will be sorted de_genes with de_genes["K562(?)_DAD1+ctrl_1+1"][0] being most differentially expressed (lowest p-val)
        print(f"de_genes: {de_genes}")
        print(f"columns: {list(de_genes.columns)[0:10]}")
        pvals =  pd.DataFrame(adata_cov.uns['rank_genes_groups']['pvals_adj'])
        print(f"pvals: {pvals}")
        for group in de_genes:
            gene_dict[group] = de_genes[group].tolist() ##key: perturbation group, value: list of DE genes with lower index genes being more differentially expressed
        print(f"gene_dict: {gene_dict.keys()}")
