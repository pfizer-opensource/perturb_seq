...
from ..utils import map_raw_id_to_vocab_id, get_pert_flags
from .. import logger
...
class TransformerGenerator(nn.Module):
    ...
    def _encode(
        ...
        ##seems to all be of shape [batch size, number of genes (max seq length), embedding size]
        ##nn.embedding for perturbation has just 3 rows instead of 1 embedding for each unique perturbation...seems like it serves as a binary flag function for 'is perturbed'
    ...
    def pred_perturb(
        ...
        # ##for evaluation, always include all genes 
        # input_gene_ids = torch.arange(ori_gene_values.size(1), device=device) ##range(0, # of genes)
