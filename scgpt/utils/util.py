...
def compute_perturbation_metrics(
    ...
    mean_ctrl = np.array(ctrl_adata.X.mean(0)).flatten()  # (n_genes,) #mean of each gene over control cells
    ...
    true_mean_perturbed_by_condition = np.array([true_perturbed[condition2idx[c]].mean(0) for c in conditions])  # (n_conditions, n_genes) #array of actual-gene means for each condition
    true_mean_delta_by_condition = true_mean_perturbed_by_condition - mean_ctrl

    zero_rows = np.where(np.all(true_mean_perturbed_by_condition == 0, axis=1))[0].tolist()
    zero_cols = np.where(np.all(true_mean_perturbed_by_condition == 0, axis=0))[0].tolist()

    pred_perturbed = results["pred"]  # (n_cells, n_genes)
    pred_mean_perturbed_by_condition = np.array([pred_perturbed[condition2idx[c]].mean(0) for c in conditions])  # (n_conditions, n_genes) #array of predicted-gene means for each condition
...
def get_gene_names(string):
    """
    edit: given a string giving the perturbation(s): return the list of genes perturbed 
    if control condition:
        will return ["ctrl"]
    else:
        a string needs to be delineated with a "+"
    """
    if string == "ctrl":
        return ["ctrl"]
    else:
        split = string.split("+")
        return [split[i] for i in range(0, len(split)) if split[i] != "ctrl"]

def get_pert_flags(batch_data, device, batch_size, n_genes, gene_idx_map, random_shuffle, pert_pad_id=0, not_perturbed_id=1, is_perturbed_id=2):
    """ 
    edit: get pert_flags from batch_data
    """
    # pert_flags = torch.ones(batch_size, n_genes, dtype=torch.long, device=device) ##0 will be pad, 1 will indicate control, 2 will be perturbation - nn.Embedding for perturbation is only 3 vectors
    pert_flags = torch.full((batch_size, n_genes), not_perturbed_id, dtype=torch.long, device=device) ##0 will be pad, 1 will indicate control, 2 will be perturbation - nn.Embedding for perturbation is only 3 vectors
    for i in range(0, len(pert_flags)): ##0 -> batch_size
        ##batch_data.pert[i] can be 'ctrl', 'ctrl+GENE', 'GENE+ctrl', 'GENE1+GENE2' 
        if batch_data.pert[i] == "ctrl":  ##leave tensor as all ones, assumes ctrl condition is specified by just the string "ctrl"
            continue
        gene_names = get_gene_names(batch_data.pert[i]) ##edge cases: can be multiple, or just ["ctrl"]
        for gene_name in gene_names: ##get index of gene_name in original gene_name_list 
            if random_shuffle: 
                index = random.randint(0, n_genes - 1) ##give a random index as negative control
            else:
                if gene_name not in gene_idx_map: 
                    print(f"WARNING: perturbation {gene_name} not found in gene_idx_map, assigning perturbation index = {pert_pad_id}")
                    index = pert_pad_id
                else:
                    index = gene_idx_map[gene_name]
            pert_flags[i][index] = is_perturbed_id
    return pert_flags